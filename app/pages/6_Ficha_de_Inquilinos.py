...
if not st.session_state["authentication_status"]:
    ...
    with tab1:
        ...
        apto_id: str = apto_input(
            st.text_input(label="Apartamento", value=None, key=6101, max_chars=5)
        )
        ...
        imob_fone = tel_input_br(
            st.text_input(label="Telefone Imobiliária", value=None, key=6122)
        )
        ...
        if st.button("Registrar", key=6173):
            ...
            try:
                post_response = requests.post(FICHA_URL, json=ficha_data)
                show_response_message(post_response)
                if post_response.status_code == 200:
                    st.subheader("Dados inseridos, tudo OK:")
                else:
    ...
    with tab2:
        ...
        if get_id:
            get_response = requests.get(f"{FICHA_URL}{get_id}")
    ...
    with tab3:
        ...
        if update_id:
            update_response = requests.get(f"{FICHA_URL}{update_id}")
            if update_response.status_code == 200:
                ...
                with st.expander("Dados do Automóvel"):
                    automovel = st.text_input(
                        label="Automóvel",
                        value=empty_none(df_up.automovel[0]),
                        key=6314,
                    )
                ...
                proprietario = st.text_input(
                    label="Proprietário",
                    value=empty_none(df_up.proprietario[0]),
                    key=6321,
                )
                imob_fone = tel_input_br(
                    st.text_input(
                ...
                if st.button("Modificar", key=6363):
                    ...
                    try:
                        put_response = requests.put(
                            f"{FICHA_URL}{update_id}", json=ficha_up_data
                        )
    ...
    with tab4:
        ...
        if delete_id:
            show_delete_response = requests.get(f"{FICHA_URL}{delete_id}")
            if show_delete_response.status_code == 200:
                ...
                if delete_button and delete_confirm:
                    delete_response = requests.delete(f"{FICHA_URL}{delete_id}")
    ...
    with tab5:
        ...
        if st.button("Mostrar", key=6500):
            get_list_response = requests.get(FICHA_URL)
