    ...
    with tab1:
        ...
        if apto_id:
            ...
            if apto_response.status_code == 200:
                ...
                if from_day:
                    despesas_response = requests.get(DESP_URL)
                        ...
                        if despesas:
                            ...
                            if not df_desp.empty:
                                ...
                                df_desp = df_desp.drop(
                                    ["apto_id", "id", "criado_em", "modificado_em"],
                                    axis=1,
                                )
                                desp_ord = ["data", "categoria", "descricao", "valor"]
                                despesas_tot = round(df_desp["valor"].sum(), 2)
                                df_desp = showbr_dfdate(
                                    df_desp.reindex(desp_ord, axis=1)
                                )
                                df_desp.columns = [
                                    "Data",
                                    "Categoria",
                    ...
                    if alugueis_response.status_code == 200:
                        ...
                        if alugueis:
                            ...
                            if not df_alug.empty:
                                ...
                                df_alug = showbr_dfdate(
                                    df_alug.reindex(alug_ord, axis=1)
                                )
                                df_alug.columns = [
                                    "Check-In",
                                    "Check-Out",
                ...
                if from_day and not (df_desp.empty or df_alug.empty):
                    ...
                    if gen_relatorio:
                        ...
                        post_rel_response = requests.post(RELAT_URL, json=rel_reg)
