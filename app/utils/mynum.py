...
def calculate_valortotal(diarias, valor_diaria):
    if diarias is None or valor_diaria is None:
        ...
        return None
    if not isinstance(diarias, (int, float)) or not isinstance(
        valor_diaria, (int, float)
    ):
...
def cep_input(cep_in: str | int | float) -> str:
    ...
    if len(cep_num) == 8:
        ...
        st.warning("Não foi identificado o padrão brasileiro de CEP")
        return cep_in
...
def cpf_input(cpf_in: str | int | float) -> str:
    ...
    if len(cpf_num) == 11:
        ...
        st.warning("Não foi identificado o padrão brasileiro de CPF")
        return cpf_in
...
def rg_input(rg_in: str | int | float) -> str:
    ...
    match len(rg_num):
        ...
        case _:
            st.warning("Não foi identificado o padrão brasileiro de RG")
            return rg_in
...
def tel_input_br(tel_in: str | int | float) -> str:
    ...
    match len(tel_num):
        case 8:
            return f"+XX (XX) {tel_num[:4]}-{tel_num[4:]}"
        case 9:
            return f"+XX (XX) {tel_num[0]} {tel_num[1:5]}-{tel_num[5:]}"
        case 10:
        ...
        case _:
            st.warning("Não foi identificado o padrão brasileiro telefônico")
            return tel_in
